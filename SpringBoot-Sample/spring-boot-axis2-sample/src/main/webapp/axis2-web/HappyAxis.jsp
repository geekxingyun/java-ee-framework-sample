...
<body>
	...
	<table summary="header table" class="FULL_BLANK">
		<tr>
			<td>
				...
				</table> <br> <!--  head.inc end --> <jsp:include
					page="/WEB-INF/include/link-footer.jsp" /> <%
 ...
 %>
	...
	*/
	 ...
	 * @throws IOException
	 */
	int probeClass(JspWriter out, String category, String classname, String jarFile, String axisOperation,
		...
		try {
			...
			if (clazz == null) {
				...
				out.write("<p>" + category + ": could not find class " + classname + " from file <b>" + jarFile
						+ "</b><br>  " + errorText + url + "<p>");
				return 1;
			} else {
				...
				if (location == null) {
					...
					out.write("Found " + axisOperation + " (" + classname + ") <br> &nbsp;&nbsp;at " + location
							+ "<br/>");
				}
				return 0;
			}
			...
			out.write("<p>" + category + ": could not find a dependency" + " of class " + classname + " from file <b>"
					+ jarFile + "</b><br> " + errorText + url + "<br>The root cause was: " + ncdfe.getMessage()
					+ "<br>This can happen e.g. if " + classname + " is in"
					+ " the 'common' classpath, but a dependency like "
					+ " activation.jar is only in the webapp classpath." + "<p>");
			return 1;
		}
	}
	 ...
	 */
	int needClass(JspWriter out, String classname, String jarFile, String axisOperation, String errorText,
			String homePage) throws IOException {
		return probeClass(out, "<b>Error</b>", classname, jarFile, axisOperation, errorText, homePage);
	}
	 ...
	 * @return the number of missing libraries (0 or 1)
	 */
	int wantClass(JspWriter out, String classname, String jarFile, String axisOperation, String errorText,
			String homePage) throws IOException {
		return probeClass(out, "<b>Warning</b>", classname, jarFile, axisOperation, errorText, homePage);
	}
	 ...
	 * @throws Exception
	 */
	int wantResource(JspWriter out, String resource, String errorText) throws Exception {
	...
	public boolean invokeTheService() {
		try {
			...
			ConfigurationContext configctx = ConfigurationContextFactory.createConfigurationContextFromFileSystem(null,
					null);
	...
	public String getFormatedSystemProperty(String systemProperty) {
 	 ...
 	 */
 	needed = needClass(out, "org.apache.axis2.transport.http.AxisServlet", "axis2-1.0.jar", "Apache-Axis",
 			"Axis2 will not work", "http://xml.apache.org/axis2/");
 	needed += needClass(out, "org.apache.commons.logging.Log", "commons-logging.jar", "Jakarta-Commons Logging",
 			"Axis2 will not work", "http://jakarta.apache.org/commons/logging.html");
 	needed += needClass(out, "javax.xml.stream.XMLStreamReader", "stax-api-1.0.1.jar", "Streaming API for XML",
 			"Axis2 will not work", "http://dist.codehaus.org/stax/jars/");
 	needed += needClass(out, "org.codehaus.stax2.XMLStreamWriter2", "wstx-asl-3.0.1.jar",
 ...
 %> <%
 	...
 	*/
 	...
 	if (needed == 0) {
 		...
 		out.write("<p style=\"color:red; font-style:italic\">" + needed + " core axis2 librar"
 				+ (needed == 1 ? "y is" : "ies are") + " missing</p>");
 	}
 	//now look at wanted stuff
 %>
				<p>
					<span style="font-style: bold italic">Note:</span> Even if
					everything this page probes for is present, there is no guarantee
					your Axis Service will work, because there are many configuration
					options that we do not check for. These tests are <span
						style="font-style: italic">necessary</span> but not <span
 ...
 %>
 	...
 	if (e != null) {
 		out.write(
 				"<table summary=\"main content table\" cellpadding=\"5\" cellspacing=\"0\" style=\"border: .5 blue solid;\">");
 ...
 %> <%-- <jsp:include page="/WEB-INF/include/footer.inc"/> --%> <!-- footer.inc start -->
				<!--
  ~ Licensed to the Apache Software Foundation (ASF) under one
  ~ or more contributor license agreements. See the NOTICE file
  ~ distributed with this work for additional information
